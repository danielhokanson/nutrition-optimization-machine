// File: Nom.Api/_Abstractions/_Events/_EventBus.cs
...
namespace Nom.Api._Abstractions._Events
{
    ...
    /// </summary>
    public class _EventBus : _IEventBus
    {
        private readonly ILogger<_EventBus> _logger;
        private readonly _EventBusOptions _options;
        private readonly ConcurrentDictionary<Type, List<_EventHandlerInfo>> _subscribers;
        private readonly ConcurrentDictionary<Type, List<_DelegateHandlerInfo>> _delegateSubscribers;
        private readonly SemaphoreSlim _semaphore;
        private readonly _EventBusStatistics _statistics;
        private bool _enabled;

        public _EventBus(ILogger<_EventBus> logger, IOptions<_EventBusOptions> options)
        {
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
            _options = options?.Value ?? new _EventBusOptions();
            _subscribers = new ConcurrentDictionary<Type, List<_EventHandlerInfo>>();
            _delegateSubscribers = new ConcurrentDictionary<Type, List<_DelegateHandlerInfo>>();
            _semaphore = new SemaphoreSlim(_options.MaxConcurrentHandlers, _options.MaxConcurrentHandlers);
            _statistics = new _EventBusStatistics();
        ...
        public async Task PublishAsync<TEvent>(TEvent @event) where TEvent : _IEvent
        {
            if (!_enabled)
            {
        ...
        public void Subscribe<TEvent>(_IEventHandler<TEvent> handler) where TEvent : _IEvent
        {
            if (handler == null)
            {
            ...
            var eventType = typeof(TEvent);
            var handlerInfo = new _EventHandlerInfo
            {
                Handler = handler,
            ...
            _subscribers.AddOrUpdate(
                eventType,
                new List<_EventHandlerInfo> { handlerInfo },
                (key, existing) =>
                {
        ...
        public void Unsubscribe<TEvent>(_IEventHandler<TEvent> handler) where TEvent : _IEvent
        {
            if (handler == null)
            {
        ...
        public void Subscribe<TEvent>(Func<TEvent, Task> handler, int priority = 0) where TEvent : _IEvent
        {
            if (handler == null)
            {
            ...
            var eventType = typeof(TEvent);
            var handlerInfo = new _DelegateHandlerInfo
            {
                Handler = handler,
                Priority = priority
            };
            ...
            _delegateSubscribers.AddOrUpdate(
                eventType,
                new List<_DelegateHandlerInfo> { handlerInfo },
                (key, existing) =>
                {
                    existing.Add(handlerInfo);
                    return existing;
        ...
        public void Unsubscribe<TEvent>(Func<TEvent, Task> handler) where TEvent : _IEvent
        {
            if (handler == null)
            {
        ...
        public int GetSubscriberCount<TEvent>() where TEvent : _IEvent
        {
            var eventType = typeof(TEvent);
            var count = 0;
        ...
        public void ClearSubscribers<TEvent>() where TEvent : _IEvent
        {
            var eventType = typeof(TEvent);
            _subscribers.TryRemove(eventType, out _);
            _delegateSubscribers.TryRemove(eventType, out _);
        ...
        public _EventBusStatistics GetStatistics()
        {
            return new _EventBusStatistics
            {
        ...
        private async Task ExecuteHandlerAsync<TEvent>(_EventHandlerInfo handlerInfo, TEvent @event) where TEvent : _IEvent
        {
                ...
                try
                {
                    ...
                    try
                    {
                        if (_options.HandlerTimeoutSeconds > 0)
                        {
                            ...
                            await handlerInfo.Handler.HandleAsync(@event).WaitAsync(cts.Token);
                        }
                        else
                        {
                            await handlerInfo.Handler.HandleAsync(@event);
                        }
        ...
        private async Task ExecuteDelegateHandlerAsync<TEvent>(_DelegateHandlerInfo handlerInfo, TEvent @event) where TEvent : _IEvent
        {
            var startTime = DateTime.UtcNow;
            var retryCount = 0;
            ...
            while (retryCount <= _options.MaxRetryAttempts)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        {
                            using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(_options.HandlerTimeoutSeconds));
                            await handlerInfo.Handler(@event).WaitAsync(cts.Token);
                        }
                        else
                        {
        ...
        private bool ValidateEvent(_IEvent @event)
        {
    ...
    /// </summary>
    internal class _EventHandlerInfo
    {
        public _IEventHandler<_IEvent> Handler { get; set; } = null!;
    ...
    /// </summary>
    internal class _DelegateHandlerInfo
    {
