...
BEGIN
    -- Fetch MeasurementType IDs dynamically
    ...
    -- 3. Insert into nutrient.IngredientNutrient
    -- Join with recipe.Ingredient and nutrient.Nutient using their FdcId to get internal IDs.
    -- Crucially, join with nutrient."Nutrient" to get the MeasurementTypeId based on the nutrient's unit.
    -- Added DISTINCT ON to handle potential duplicate (IngredientId, NutrientId) pairs within the fdc_food_nutrient_staging itself.
    INSERT INTO nutrient."IngredientNutrient" ("IngredientId", "NutrientId", "Amount", "MeasurementTypeId", "FdcId", "CreatedDate", "CreatedByPersonId", "LastModifiedDate", "LastModifiedByPersonId")
    SELECT DISTINCT ON (i."Id", n."Id") -- Ensure only one row per unique (IngredientId, NutrientId) pair
        i."Id",
        n."Id",
        NULLIF(TRIM(fns.amount), '')::DECIMAL(18,4), -- Cast amount from TEXT to DECIMAL
        n."DefaultMeasurementTypeId", -- Get MeasurementTypeId from the Nutrient table
        TRIM(fns.id), -- Store the FDC food_nutrient ID for traceability
    ...
    WHERE NULLIF(TRIM(fns.amount), '')::DECIMAL(18,4) IS NOT NULL AND NULLIF(TRIM(fns.amount), '')::DECIMAL(18,4) >= 0
    ORDER BY i."Id", n."Id", fns.id -- Order to ensure consistent selection for DISTINCT ON (e.g., pick by fns.id)
    ON CONFLICT ("IngredientId", "NutrientId") DO UPDATE SET -- Update if the combination of IngredientId and NutrientId conflicts
        "Amount" = EXCLUDED."Amount", -- Use the new amount from the CSV
    ...
    RAISE NOTICE 'Processed food nutrient associations into nutrient.IngredientNutrient table.';
